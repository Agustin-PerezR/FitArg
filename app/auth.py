...
def register_user(name: str, email: str, password: str, db_path=None):
    ...
    cursor.execute("INSERT INTO users (name, email, password_hash, is_active) VALUES (?, ?, ?, 1)", (name, email, pwd_hash))
    user_id = cursor.lastrowid
    conn.commit()
    conn.close()
    return {"success": True, "status_code": 201, "user": {"id": user_id, "name": name, "email": email}, "message": "Usuario registrado exitosamente."}
...
def login_user(email: str, password: str, db_path=None):
    ...
    return {"success": True, "status_code": 200, "token": token, "user": {"id": user["id"], "name": user["name"], "email": user["email"]}}
