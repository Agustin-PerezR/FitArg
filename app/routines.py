...
def get_user_routines(user_id: int, db_path=None):
    """Obtiene la lista de rutinas personales creadas por el usuario."""
    if not user_id:
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, user_id, name, description, exercises_json, created_at, updated_at FROM routines WHERE user_id = ? ORDER BY updated_at DESC",
        (user_id,)
    )
...
def get_routine_by_id(routine_id: int, user_id: int, db_path=None):
    """Obtiene el detalle completo de una rutina personal perteneciente al usuario."""
    ...
    cursor = conn.cursor()
    cursor.execute(
        "SELECT id, user_id, name, description, exercises_json, created_at, updated_at FROM routines WHERE id = ? AND user_id = ?",
        (routine_id, user_id)
    )
    row = cursor.fetchone()
    conn.close()
...
def save_routine(user_id: int, name: str, description: str, exercises: list, routine_id: int = None, db_path=None):
    """Crea una nueva rutina personal o actualiza una existente."""
    if not user_id:
        return {"success": False, "status_code": 400, "error": "ID de usuario requerido."}
    ...
    if routine_id:
        ...
        cursor.execute(
            "UPDATE routines SET name = ?, description = ?, exercises_json = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ? AND user_id = ?",
            (name, description, exercises_json, routine_id, user_id)
        )
        conn.commit()
        conn.close()
        return {
        ...
        # Crear nueva rutina
        cursor.execute(
            "INSERT INTO routines (user_id, name, description, exercises_json) VALUES (?, ?, ?, ?)",
            (user_id, name, description, exercises_json)
        )
...
def delete_routine(routine_id: int, user_id: int, db_path=None):
    """Elimina una rutina personal del usuario."""
    if not routine_id or not user_id:
        return {"success": False, "status_code": 400, "error": "ID de rutina y usuario requeridos."}
